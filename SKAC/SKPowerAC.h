...
private:
    ...
    float pw_b = 1;
public:
    SKPowerAC(int mem, int bucket_num, int bucket_size, int depth,float a,float b){
        // init estimator
        ss_depth = depth;
        bkt_num = bucket_num;
               ...
               // cout<<"choise 0, res: "<<pvote[res]<<endl;
                ...
                if(key[res] == flowID){
                 ...
                 //   cout<<"choise 1, res: "<<pvote[res]<<endl;
                    ...
                    if((rand()/double(RAND_MAX))<pow((pw_b*1.0/pvote[res]), pw_a)){
                        ACEstInsert(NAC[res],elemID);
                        nvote[res] = UpdateVote(NAC[res]);
                   //     cout<<"choise 2, res: "<<pvote[res]<<endl;
